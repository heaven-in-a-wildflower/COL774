...
class MultiClassNN:
    def __init__(self, layer_sizes):
        ...
        # Ensure weights and biases use float64
        self.weights = [np.random.randn(x, y).T * np.sqrt(2. / x).astype(np.float64)
                        for x, y in zip(layer_sizes[:-1], layer_sizes[1:])]
        self.biases = [np.zeros((y, 1), dtype=np.float64) for y in layer_sizes[1:]]
    ...
    def forward_pass(self, x):
        ...
        for i, (w, b) in enumerate(zip(self.weights, self.biases)):
            z = np.dot(w, a) + b
            zs.append(z)
            if i == self.num_layers - 2:  # Last layer uses Softmax
                a = self.softmax(z)
    ...
    def backward_pass(self, x, y):
        ...
        # Convert scalar y to one-hot encoded vector
        y_one_hot = np.zeros((self.layer_sizes[-1], 1), dtype=np.float64)
        y_one_hot[int(y)] = 1.0
        
        delta = activations[-1] - y_one_hot
        
        nabla_b = [np.zeros(b.shape, dtype=np.float64) for b in self.biases]
        nabla_w = [np.zeros(w.shape, dtype=np.float64) for w in self.weights]
    ...
    def evaluate(self, test_data):
        test_results = [np.argmax(self.predict(x)) for (x, _) in test_data]

        true_results = [np.argmax(y) for (_, y) in test_data]
        
        correct_predictions = sum(int(pred == true) for pred, true in zip(test_results, true_results))
        # Calculate accuracy
        return correct_predictions / len(test_data)

# Argument parser for command-line arguments
def main():
    parser = argparse.ArgumentParser(description="Train a multi-class classification neural network.")
    ...
    training_data = [(images.reshape(-1, 1).astype(np.float64), labels.reshape(-1, 1).astype(np.float64)) for images, labels in train_dataloader]
    #validation_data = [(images.reshape(-1, 1).astype(np.float64), labels.reshape(-1, 1).astype(np.float64)) for images, labels in val_dataloader]

    # Initialize and train the model
    layer_sizes = [625, 512, 256, 128, 8]  # Last layer is for 8-class classification
