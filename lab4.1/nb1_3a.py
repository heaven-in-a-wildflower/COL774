...
# Function to preprocess the text (lowercasing, stopword removal, stemming, and generating uni-grams and bi-grams)
...
class NaiveBayesClassifier:
    def __init__(self, model_type='multinomial'):
        ...
        self.absence_probs = defaultdict(lambda: defaultdict(float))  # New attribute for absence probabilities
    ...
    def train(self, texts, labels):
        ...
        # Count words (including uni-grams and bi-grams) and classes
        ...
        for word in self.vocab:
            for label in class_counts:
                ...
                self.absence_probs[label][word] = np.log(1 - np.exp(self.word_probs[label][word]))
    ...
    #     words = set(text) if self.model_type == 'bernoulli' else text
    ...
    def predict(self, text):
        ...
        vocab_words = set(self.vocab)
        present_words = vocab_words.intersection(words)
        absent_words = vocab_words - present_words if self.model_type == 'bernoulli' else set()

        for idx, label in enumerate(self.class_labels):
            score = self.class_probs[label]

            if self.model_type == 'multinomial':
                # Vectorized operation for present words
                score += sum(self.word_probs[label].get(word, np.log(1 / (len(self.vocab) + self.class_word_counter[label]))) for word in words)
            else:  # Bernoulli model
                # Vectorized operations for present and absent words
                score += sum(self.word_probs[label][word] for word in present_words)
                score += sum(self.absence_probs[label][word] for word in absent_words)
                score += np.log(1 / (2 + self.class_counter[label]))
                # # Handle words not in vocabulary
                # unknown_words_count = len(words) - len(present_words)
                # if unknown_words_count > 0:
                #     score += unknown_words_count * np.log(1 / (2 + self.class_counter[label]))
...
parser.add_argument("--model", type=str, required=False, default="bernoulli", help="Choose either 'bernoulli' or 'multinomial'")
...
# Predict labels for the test data with debug information
predictions = []
for i, text in enumerate(test_df['processed_text'].tolist()):
    # Predict the class scores for the current text
    class_scores = nb.predict(text)  # This will give log probabilities for each class
    predicted_class_idx = np.argmax(class_scores)  # Get the index of the highest score
    predicted_class = nb.class_labels[predicted_class_idx]  # Map index to class label
...
# Now save the predictions to the output file
